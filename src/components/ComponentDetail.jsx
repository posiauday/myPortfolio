    ...
    switch (tab) {
            ...
            {previewView === "mock" ? (
              <div className="mt-5 grid gap-5 rounded-[32px] bg-slate-100 p-5 dark:bg-white/5 lg:grid-cols-[1.3fr_.7fr]">
                ...
                <aside className="rounded-[26px] bg-white p-6 dark:bg-[#17201B]">
                  ...
                  <div className="mt-5 rounded-2xl p-4 text-sm" style={{ background: `${item.color}14` }}>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {item.variants.map((v, i) => (
                <div key={v} className="rounded-2xl border border-slate-200 p-5 dark:border-white/10">
                  ...
                  <p className="mt-2 text-sm text-slate-500">{v === "Compact" ? "Reduced density for galleries and constrained layouts." : v === "Dark" ? "Token-adjusted surfaces, borders and readable states." : v === "Mobile" ? "Narrow layout with touch-friendly actions and stacking." : "Full information layout for primary screens."}</p>
                </div>
              ))}
            </div>
          </Panel>
          ...
          <Panel title="Properties">
            <div className="mt-6 overflow-x-auto">
              <div className="min-w-[680px]">
                <div className="grid grid-cols-[1fr_.7fr_.8fr_1.7fr] gap-3 border-b border-slate-200 pb-3 text-xs font-black uppercase tracking-wider text-slate-400 dark:border-white/10"><span>Property</span><span>Type</span><span>Default</span><span>Description</span></div>
                {item.properties.map(([name, type, def, desc]) => (
                  <div key={name} className="grid grid-cols-[1fr_.7fr_.8fr_1.7fr] gap-3 border-b border-slate-100 py-4 text-sm dark:border-white/10">
                    <b>{name}</b><span className="text-slate-500">{type}</span><code className="text-xs" style={{ color: item.color }}>{def}</code><span className="text-slate-600 dark:text-slate-300">{desc}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      case "Events":
        return (
          <Panel title="Events">
            {item.events.length ? (
              <div className="mt-6 grid gap-3">{item.events.map(([name, desc]) => <div key={name} className="rounded-2xl bg-slate-50 p-5 dark:bg-white/10"><code className="font-black" style={{ color: item.color }}>{name}</code><p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{desc}</p></div>)}</div>
            ) : (
              <div className="mt-6 rounded-2xl bg-slate-50 p-6 dark:bg-white/10"><b>No behavior events</b><p className="mt-2 text-sm text-slate-500">This visual is input-driven and exposes no interaction event in the current specification.</p></div>
            )}
          </Panel>
        );
      case "Architecture":
      ...
      case "Examples":
        return (
          <Panel title="Examples">
            <div className="mt-6 grid gap-4 md:grid-cols-3">{item.examples.map((x, i) => <div key={x} className="rounded-2xl bg-slate-50 p-5 dark:bg-white/10"><span className="text-xs font-black" style={{ color: item.color }}>EXAMPLE {String(i + 1).padStart(2, "0")}</span><h3 className="mt-3 text-lg font-black">{x}</h3><p className="mt-2 text-sm text-slate-500">Use the property contract while the host app owns data access, security and persistence.</p></div>)}</div>
          </Panel>
        );
      case "Accessibility":
        return (
      ...
      case "Limitations":
        return (
          <Panel title="Limitations">
            ...
            <p className="mt-6 text-xs leading-5 text-slate-500">Blueprint and Original entries describe a design contract. They are not presented as downloadable production YAML until their definitions are built and tested in Power Apps Studio.</p>
          </Panel>
        );
      default:
        return null;
    ...
    <main className={dark ? "dark min-h-screen bg-[#0B1110] text-white" : "min-h-screen bg-[#FBFDFB] text-[#17201B]"}>
      ...
      <div className="mx-auto max-w-7xl px-5 py-10">
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full px-3 py-1 text-xs font-black" style={{ background: `${item.color}18`, color: item.color }}>{item.maturity}</span>
